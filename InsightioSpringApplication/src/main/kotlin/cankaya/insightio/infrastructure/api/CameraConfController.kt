...
class CameraController(private val cameraService: CameraService) {
    ...
    @GetMapping("/{id}")
    fun getCameraById(@PathVariable id: String): ResponseEntity<Camera> =
        cameraService.findCameraById(id)?.let { ResponseEntity.ok(it) } ?: ResponseEntity.notFound().build()

    @PostMapping
    fun createCamera(@RequestBody camera: Camera): ResponseEntity<Camera> =
        ResponseEntity.ok(cameraService.createCamera(camera))

    @PutMapping("/{id}")
    fun updateCamera(@PathVariable id: String, @RequestBody camera: Camera): ResponseEntity<Camera> =
        ResponseEntity.ok(cameraService.updateCamera(camera))
    ...
    fun getCamerasByType(@PathVariable type: CameraType): ResponseEntity<List<Camera>> {
    ...
    fun getCamerasByStatus(@PathVariable status: CameraStatus): ResponseEntity<List<Camera>> {
        val cameras = cameraService.findByStatus(status)
        return if (cameras.isNotEmpty()) ResponseEntity.ok(cameras) else ResponseEntity.notFound().build()
    }

    @GetMapping("/name/{name}")
    fun getCameraByName(@PathVariable name: String): ResponseEntity<Camera> {
